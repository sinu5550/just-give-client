...
const fetchUserProfile = async (user_id) => {
    try {
        const response = await fetch(`https://justgive-api-sinu5550.onrender.com/user/profile/${user_id}/`);
        if (!response.ok) {
...
const handleSaveCHanges = async (event) => {
    ...
    if (profileImageInput.files.length > 0) {
        ...
        try {
            const imgResponse = await fetch(`https://justgive-api-sinu5550.onrender.com/user/profile/${user_id}/`, {
                method: "PATCH",
                body: formData,
            });
